 ...
 * <p>
 * 从 1.21.2 起原版把流体移动拆成了独立方法:{@code travel} 只负责入口判断
 * ({@code isTouchingWater() || isInLava()} 即进入 {@code travelInFluid}),真正的水/岩浆
...
public abstract class LivingEntity_lavaDepthStriderMixin {
	...
	private boolean hao$lavaActsAsWater(LivingEntity entity, Operation<Boolean> original) {
		...
		return player.getAttributeValue(EntityAttributes.WATER_MOVEMENT_EFFICIENCY) > 0.0;
	}
}
