...
public final class RegionObserverRefresh {
	...
	public static int startLoadedFaceToFacePairs(ServerWorld world, DetectorRegion region) {
		int startedPairs = 0;
	...
	private static int startPairsInChunk(ServerWorld world, DetectorRegion region, WorldChunk chunk) {
		int startedPairs = 0;
		...
		int bottomSection = ChunkSectionPos.getSectionCoord(chunk.getBottomY());
		ChunkSection[] sections = chunk.getSectionArray();
		int topSection = bottomSection + sections.length - 1;
		int minSection = Math.max(ChunkSectionPos.getSectionCoord(region.min().getY()), bottomSection);
		int maxSection = Math.min(ChunkSectionPos.getSectionCoord(region.max().getY()), topSection);

		for (int sectionIndex = minSection; sectionIndex <= maxSection; sectionIndex++) {
			ChunkSection section = sections[sectionIndex - bottomSection];
			if (section.isEmpty() || !section.hasAny(state -> state.isOf(Blocks.OBSERVER))) {
				continue;
			}
	...
	private static boolean startPairIfNeeded(ServerWorld world, DetectorRegion region, BlockPos pos) {
		...
		if (!region.contains(world.getRegistryKey(), otherPos)
				|| !world.getChunkManager().isChunkLoaded(otherPos.getX() >> 4, otherPos.getZ() >> 4)) {
			return false;
		}

		BlockState otherState = world.getBlockState(otherPos);
		if (!otherState.isOf(Blocks.OBSERVER) || otherState.get(FacingBlock.FACING) != facing.getOpposite()) {
			return false;
		}
		...
		world.getBlockTickScheduler().scheduleTick(OrderedTick.create(Blocks.OBSERVER, pos));
		world.getBlockTickScheduler().scheduleTick(OrderedTick.create(Blocks.OBSERVER, otherPos));
		return true;
	}
}
