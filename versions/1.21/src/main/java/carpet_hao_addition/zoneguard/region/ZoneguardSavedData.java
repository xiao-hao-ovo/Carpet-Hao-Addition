...
public class ZoneguardSavedData extends PersistentState {
	public static final String DATA_NAME = "zoneguard_detector_regions";
	...
	public static final PersistentState.Type<ZoneguardSavedData> TYPE = new PersistentState.Type<>(
			ZoneguardSavedData::new,
			ZoneguardSavedData::fromNbt,
			DataFixTypes.LEVEL
	);
	...
	private static ZoneguardSavedData fromNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup lookup) {
		return CODEC.parse(NbtOps.INSTANCE, nbt).result().orElseGet(ZoneguardSavedData::new);
	}
