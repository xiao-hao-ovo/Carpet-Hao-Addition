...
public record DetectorRegion(RegistryKey<World> dimension, BlockPos min, BlockPos max) {
	public static final Codec<DetectorRegion> CODEC = RecordCodecBuilder.create(instance -> instance.group(
			RegistryKey.createCodec(RegistryKeys.WORLD).fieldOf("dimension").forGetter(DetectorRegion::dimension),
	...
	public static DetectorRegion fromCorners(RegistryKey<World> dimension, BlockPos first, BlockPos second) {
		return new DetectorRegion(dimension, BlockPos.min(first, second), BlockPos.max(first, second));
	}

	public boolean contains(RegistryKey<World> levelDimension, BlockPos pos) {
