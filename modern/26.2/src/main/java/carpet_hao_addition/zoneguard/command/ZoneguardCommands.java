...
/**
 * /zoneguard 命令树(1.21.10 版;1.21.10 把玩家/名单类型换成 NameAndId)。
 * <p>
...
public final class ZoneguardCommands {
	...
	private static int view(CommandContext<CommandSourceStack> context) {
		...
		context.getSource().sendSuccess(() -> msg("view.header"), false);
		data.regions().forEach((id, region) -> context.getSource().sendSuccess(
	...
	private static int help(CommandContext<CommandSourceStack> context) {
		...
		source.sendSuccess(() -> msg("help.line.help"), false);
		source.sendSuccess(() -> msg("help.line.set"), false);
		source.sendSuccess(() -> msg("help.line.view"), false);
		source.sendSuccess(() -> msg("help.line.clear"), false);
		source.sendSuccess(() -> msg("help.line.op"), false);
		source.sendSuccess(() -> msg("help.line.off"), false);
		return 1;
	}
	...
	private static int addOperator(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
		...
		for (NameAndId entry : entries) {
			if (data.addOperator(entry)) {
				added++;
				source.sendSuccess(() -> msg("op.add.success", entry.name()), true);
			} else {
				source.sendSuccess(() -> msg("op.add.exists", entry.name()), false);
			}
		}
	...
	private static int removeOperator(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
		...
		for (NameAndId entry : entries) {
			...
			if (removedName.found()) {
				removed++;
				source.sendSuccess(() -> msg("op.remove.success", removedName.name()), true);
			} else {
				source.sendFailure(msg("op.remove.notFound", entry.name()));
			}
		}
	...
	private static OptionalName removeOperator(ZoneguardSavedData data, NameAndId entry) {
	...
	private static int listOperators(CommandContext<CommandSourceStack> context) {
		...
		context.getSource().sendSuccess(() -> msg("op.list", joiner.toString()), false);
	...
	private static String regionDimension(DetectorRegion region) {
		return String.valueOf(region.dimension().identifier());
	}
