**Carpet-Hao-Addition** 是一个 Fabric Carpet 扩展模组，内置一组可开关的地毯规则（**全部默认关闭**），覆盖 Minecraft **1.21 ~ 26.2**；只依赖 **Fabric Loader + Carpet**（刻意不依赖 fabric-api），可纯服务端运行。

**Carpet-Hao-Addition** is a Fabric Carpet extension that bundles a set of toggleable Carpet rules — **all disabled by default** — covering Minecraft **1.21 through 26.2**. It deliberately depends on nothing but **Fabric Loader + Carpet** (no fabric-api), so it works on a pure server.

- **规则 / Rules**：区域禁侦测器 / zone guard（在设定区域内禁用侦测器）、金胡萝卜堆肥 / golden-carrot composting、雪地方解石 / snowy calcite、末地门传送白名单 / end-portal teleport whitelist、陶瓦去色 / terracotta un-dyeing、基岩可挖 / mineable bedrock、凋灵骷髅掉落削减 / wither-skeleton drop reduction。游戏内用 `/carpet <规则> <值>` 切换（分类 **Hao**）。
- **下载 / Download**：请下载与你的 Minecraft 版本**完全对应**的 jar —— 每个 jar 只针对一个版本（1.21, 1.21.1, 1.21.2, 1.21.4, 1.21.6, 1.21.8, 1.21.10, 1.21.11, 26.1.2, 26.2）。Pick the jar matching your exact Minecraft version — each jar targets a single version.
- **依赖 / Requirements**：Fabric Loader ≥ 0.16.10 及对应版本的 Carpet；1.21.x 需 Java 21+，26.x 需 **Java 25**。Fabric Loader ≥ 0.16.10 and the matching Carpet release; Java 21+ for 1.21.x, **Java 25** for 26.x.
- **源码 / Source**：https://github.com/xiao-hao-ovo/Carpet-Hao-Addition
